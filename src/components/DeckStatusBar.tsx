...
export default function DeckStatusBar({ deckId, status, shareToken, cardCount }: DeckStatusBarProps) {
  ...
  return (
    <div data-testid="deck-status-bar">
          ...
          {status === 'active' && (
            <>
              ...
              <button
                ...
                className="rounded-lg border border-primary/15 px-3 py-1.5 text-xs font-medium text-primary/50 hover:border-primary/30 transition-colors"
      ...
      {showQR && status === 'active' && (
        <div
          className="mt-4 flex flex-col items-center gap-3 rounded-xl border border-primary/10 bg-white p-6"
          data-testid="qr-code-section"
        >
          <QRCode value={shareUrl} size={180} />
          <p className="text-xs text-primary/40 text-center max-w-xs break-all">
            {shareUrl}
          </p>
        </div>
      )}
